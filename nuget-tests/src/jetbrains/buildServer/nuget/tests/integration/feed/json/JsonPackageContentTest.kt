...
class JsonPackageContentTest : JsonFeedIntegrationTestBase() {
    ...
    fun get_package_nupkg() {
        ...
        Assert.assertTrue(redirectUrl.endsWith(DOWNLOAD_URL), "Actual redirect URL: $redirectUrl")
    }
    ...
    fun get_package_nuspec() {
        ...
        val redirectUrl = response.getHeader("Location")
        Assert.assertTrue(redirectUrl.endsWith("$DOWNLOAD_URL!/MyPackage.nuspec"), "Actual redirect URL: $redirectUrl")
    }
