...
class PublishPackageServiceMessageHandlerTest : BaseTestCase() {
    ...
    fun shouldPublishPackage() {
        ...
        val workingDirectory = myTempArtifactsDir
        ...
        m.checking(object: Expectations() {
            init {
                oneOf(runner).workingDirectory
                will(returnValue(workingDirectory))

                oneOf(myPackageAnalyzer).analyzePackage(with(any(InputStream::class.java)))
                will(returnValue(metadata))

                oneOf(myPackagePublisher).publishPackages(withNotNull(createPackagesMatcher(listOf(File(workingDirectory, filePath).path)), emptyList()))
            }
        })
            ...
            init {
                ...
                oneOf(myPackagePublisher).publishPackages(withNotNull(
                        createPackagesMatcher(
                                packages.map { File(workingDirectory, it.path).path }.toList()
                        ),
                        emptyList()))
            }
        })
    ...
    fun shouldGroupSamePackages() {
        ...
        m.checking(object: Expectations() {
            init {
                ...
                oneOf(myPackagePublisher).publishPackages(withNotNull(
                        createPackagesMatcher(
                                packages.map { File(workingDirectory, it.path).path }.distinct().toList()
                        ),
                        emptyList()))
            }
        })
    ...
    fun shouldClearPackagesListOnRunnerFinished(buildFinishStatus: BuildFinishedStatus) {
        ...
        m.checking(object: Expectations() {
            init {
                ...
                allowing(myPackagePublisher).publishPackages(withNotNull(createPackagesMatcher(listOf(File(workingDirectory, filePath).path)), emptyList()))
            }
        })
