...
import jetbrains.buildServer.nuget.feed.server.NuGetFeedConstants.NUGET_FEED_ASYNC_V3_REGISTRATIONS
...
@RestController
@RequestMapping(NUGET_FEED_ASYNC_V3_REGISTRATIONS)
class AsyncJsonRegistrationController(
    ...
    fun getRegistration(
        ...
        return createAsyncTask<ResponseEntity<String>>(Callable
        {
            val context = request.getAttribute(NuGetFeedConstants.NUGET_FEED_ASYNC_DATA_CONTEXT) as JsonNuGetFeedContext?
            if (context == null) {
    ...
    private fun <T> createAsyncTask(callable: Callable<T>): WebAsyncTask<T> {
        return WebAsyncTask(TeamCityProperties.getLong(PROP_NUGET_FEED_ASYNC_REQUEST_TIMOEUT,30000), asyncTaskExecutor, callable)
    }
}
