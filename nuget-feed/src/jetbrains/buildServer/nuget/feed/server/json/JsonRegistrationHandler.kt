...
class JsonRegistrationHandler(private val feedFactory: NuGetFeedFactory,
    ...
    override fun handleRequest(feedData: NuGetFeedData, request: HttpServletRequest, response: HttpServletResponse) {
        ...
        if (matchResult != null) {
            ...
            request.setAttribute("nuget.feed.json.data", context)

            val asyncEnabled = true
            if (resource == "index") {
                if (asyncEnabled) {
                    val dispatcher = request.getRequestDispatcher("/app/nuget/v3/registrations/${id}")
                ...
                if (asyncEnabled) {
                    val dispatcher = request.getRequestDispatcher("/app/nuget/v3/registrations/${id}/${resource}")
                    dispatcher.forward(request, response)
                } else {
