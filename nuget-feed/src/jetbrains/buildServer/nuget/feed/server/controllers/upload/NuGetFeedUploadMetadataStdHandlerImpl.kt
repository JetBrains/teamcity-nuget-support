...
class NuGetFeedUploadMetadataStdHandlerImpl(
    private val myStorage: MetadataStorage,
    private val myPermissionChecker: NuGetFeedPermissionChecker
) : NuGetFeedUploadMetadataHandler<NuGetFeedUploadHandlerStdContext> {
    override fun validate(
        request: MultipartHttpServletRequest,
        response: HttpServletResponse,
        context: NuGetFeedUploadHandlerStdContext,
        build: RunningBuildEx,
        key: String,
        metadata: MutableMap<String, String>
    ) {
        if (!TeamCityProperties.getBoolean(NuGetFeedConstants.PROP_NUGET_FEED_ENABLE_CROSS_PROJECT_ACCESS) && !myPermissionChecker.canWrite(build, context.feedData)) {
        ...
        if (!replace && myStorage.getEntriesByKey(context.feedData.key, key).hasNext()) {
            throw PackageExistsException(
                "NuGet package ${metadata.get(NuGetPackageAttributes.ID)}:${metadata.get(NuGetPackageAttributes.NORMALIZED_VERSION)} already exists " +
    ...
    override fun handleMetadata(
        ...
        try {
            ...
            LOG.warnAndDebugDetails(
                "Failed to update metadata for build $build in project ${context.feedData.projectExtId} feed ${context.feedData.feedId}. Error: ${e.message}",
                e
            )
            throw e
        } finally {
