...
public class AntPatternUtil
  {
    ...
    public static boolean IsFileNameMatch(String pattern, String name)
    {
      name = name.trim();
      name = name.replace('\\', '/');
      ...
      for (String s : splitted)
      {
        ...
        {
          nameComponents.add(s);
        }
      }

      AntPatternState.MatchResult matchResult = AntPatternState.MatchResult.NO;
      ...
      for (int i = 0; i < nameComponents.size(); i++)
      {
        ...
        final AntPatternState.AntPatternStateMatch enter = state.Enter(nameComponents.get(i));
        matchResult = enter.getResult();
        state = enter.getState();
      }

      return matchResult == AntPatternState.MatchResult.YES;
    }

    public static List<Wildcard> ParsePattern(String pattern, boolean caseSensitive)
    {
    ...
    private static void RemoveOneDot(List<Wildcard> wildcards)
    {
      int i = 0;
      while (i < wildcards.size() - 1)
      {
        if (wildcards.get(i) != null && wildcards.get(i).Pattern().equals("."))
          wildcards.remove(i);
        else
          i++;
      }
    ...
    private static void RemoveTwoDots(List<Wildcard> wildcards)
    {
      ...
      while (i < wildcards.size() - 1)
      {
        if (wildcards.get(i + 1) != null && wildcards.get(i + 1).Pattern().equals(".."))
        {
          wildcards.remove(i);
          wildcards.remove(i);
      ...
      while (wildcards.size() > 0 && wildcards.get(0) != null && wildcards.get(0).Pattern().equals(".."))
        wildcards.remove(0);
    }
  }
