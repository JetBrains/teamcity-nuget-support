...
public class NuGetCommandBuildProcessFactoryImpl implements CommandlineBuildProcessFactory {
  ...
  public NuGetCommandBuildProcessFactoryImpl(
    @NotNull final BuildProcessFacade facade) {
    myFacade = facade;
  }
  ...
  public BuildProcess executeCommandLine(@NotNull BuildRunnerContext hostContext,
    ...
    if(!SystemInfo.isWindows) {
      String mono = context.getConfigParameters().get(DotNetConstants.MONO_JIT);
      if(!StringUtil.isEmptyOrSpaces(mono)) {
        newArgz.add(mono);
      }
    }
